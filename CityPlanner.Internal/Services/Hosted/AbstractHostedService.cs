...
public abstract class AbstractHostedService<T> : IHostedService
{
    ...
    public AbstractHostedService(IServiceScopeFactory serviceScopeFactory, TimeSpan untilStart, TimeSpan frequency)
    {
        if (untilStart < TimeSpan.FromSeconds(Constants.Limits.MINIMUM_SECOND_HOSTED_SERVICE_DELAY))
            throw new Exception($"A hosted service cannot have a minimum time shorter than {Constants.Limits.MINIMUM_SECOND_HOSTED_SERVICE_DELAY} seconds.");
    ...
    private void DoWork(object? state)
    {
        ...
        Logger.LogInformation($"{GetType().Name} job running.");

        OnRun();

        Logger.LogInformation($"{GetType().Name} job finished, next run will happen at {GetNextRunDateTime(startedAt)}.");
    }
